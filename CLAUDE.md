...
- **Todo** (`screens/TodoScreen.js`) — full TaskAtHand client: header/task CRUD, ECDs, By-Date/Insights/Events/Goals/Projects filters, event templates, AI insights, daily local notifications (8:30 AM and 4:00 PM device time).
...
- **Filter logic** (TodoScreen `byDateSections`): By Date groups only non-done tasks by `getEcdDateKey` (recurring tasks surface under today when due), ordered today → past → future with a thick divider (`byDateDivider`) between the present/past/future sections and "No date" last. Insights, Events, Goals and Projects render as full panels replacing the header list.
